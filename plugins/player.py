...
@Client.on_message(filters.command(["fplay", "fplay", f"play@{Config.BOT_USERNAME}", f"fplay@{Config.BOT_USERNAME}"]) & chat_filter)
...
@Client.on_message(filters.command(["ytplay", f"ytplay@{Config.BOT_USERNAME}"]) & admin_filter & chat_filter)
async def yt_play_list(client, m: Message):
    with suppress(MessageIdInvalid, MessageNotModified):
        if m.reply_to_message is not None and m.reply_to_message.document:
            ...
            k=await m.reply("No playList file given. search for a playlist in @DumpPlaylist to get a playlist file.")
            await delete_messages([m, k])


@Client.on_message(filters.command(["vplay", f"vplay@{Config.BOT_USERNAME}"]) & admin_filter & chat_filter)
...
async def notforu(_, m: Message):
    k = await _.send_cached_media(chat_id=m.chat.id, file_id="CAADBQADEgQAAtMJyFVJOe6-VqYVzAI", caption="You Are Not Authorized", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('⚡️Join Here', url='https://t.me/{Config.BOT_SUPPORT}')]]))
    await delete_messages([m, k])
...
async def not_chat(_, m: Message):
    if m.from_user is not None and m.from_user.id in Config.SUDO:
        ...
        buttons = [
            [
                InlineKeyboardButton('Support ', url='https://github.com/subinps/{Config.BOT_SUPPORT}"'),
                InlineKeyboardButton('Channel', url='https://t.me/{Config.BOT_CHANNEL}"'),
            ]
            ]
